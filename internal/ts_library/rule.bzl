...
def _ts_library_impl(ctx):
    ...
    # directory.
    npm_packages = (
        npm_packages_list[0] if len(npm_packages_list) == 1 else ctx.attr._empty_npm_packages
    )
...
def _ts_library_create_full_src(ctx, internal_deps, npm_packages, output_dir, for_compilation):
    ctx.actions.run(
        ...
        arguments = [
            ...
            ("|".join([
                (";".join(
                    d[TsLibraryInfo].typescript_source_files if for_compilation and TsLibraryInfo in d else d[JsLibraryInfo].javascript_source_files,
                )) +
                ":" +
                (
